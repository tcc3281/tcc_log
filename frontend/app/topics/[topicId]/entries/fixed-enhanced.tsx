  ...
  useEffect(() => {
    ...
    const fetchTopicAndEntries = async () => {
      ...
      try {
        ...
        setFilteredEntries(entriesData);        // Extract unique filter options with proper type assertions
        ...
        const weathers = [...new Set(entriesData.map((entry: Entry) => entry.weather as string).filter((weather: unknown): weather is string => typeof weather === 'string'))] as string[];
        const moods = [...new Set(entriesData.map((entry: Entry) => entry.mood as string).filter((mood: unknown): mood is string => typeof mood === 'string'))] as string[];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              Filters
              {(filterStartDate || filterEndDate || filterLocation || filterWeather || filterMood || filterVisibility) && (
        ...
        <>
          <div className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            ...
            {filteredEntries.map((entry) => (
              <div 
                key={entry.entry_id} 
                className="card topic-card entry-card bg-white dark:bg-gray-800 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-700 transition-all duration-300"
              >
                <div className="p-6">
